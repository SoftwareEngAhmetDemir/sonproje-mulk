...
export default function EstatePropsCreate(props) {
   ...
   function getBanksF() {
      axios.get(`/banks/${props.match.params.id}`).then((response) => {
         seTstate({
   ...
   const onSubmit = async (e) => {
      ...
      if (gState.uploadPictures) {
         ...
         await axios
            .post("/uploadimg/uploadimg/" + "banks", data)
            .then((res) => {
               imgpath = res.data[0].path.replace("../public", "");
               // delete old file
      ...
      axios
         .post(`/banks/${props.match.params.id}`, sendData)
         .then((res) => {
                        ...
                        <Grid item sm={3}>
                           <FormGroup className="FormGroup">
                              <FormControl>
                                 ...
                                 />
                                 <FormHelperText>{t("Banka Adını yazınız.")}</FormHelperText>
                              </FormControl>
                           </FormGroup>
                        </Grid>
                        ...
                        <Grid item sm={3}>
                           <FormGroup className="FormGroup">
                              <FormControl>
                                 ...
                                 />
                                 <FormHelperText>{t("Bankanın Ön kodunu giriniz örn. AYF-")}</FormHelperText>
                              </FormControl>
                           </FormGroup>
                        </Grid>
                        <Grid item sm={4}>
                           <FormGroup className="FormGroup">
                              <FormControl>
                                 {state.images ? <img src={state.images} style={{ width: "150px", margin: "0 auto" }} /> : ""}
                                 <UploadImgView singleImage={true} buttonText={t("Resim Seçiniz")} />
                              </FormControl>
                           </FormGroup>
                        </Grid>
                     </Grid>
