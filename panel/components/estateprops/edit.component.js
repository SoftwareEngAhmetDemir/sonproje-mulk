...
export default function EstatePropsEdit(props) {
   ...
   function getEstatePropsData() {
      axios.get(`/estateprops/${props.match.params.id}`).then((response) => {
         const details = [];
   ...
   const onSubmit = (e) => {
      ...
      axios
         .post(`/estateprops/${props.match.params.id}`, EstateProps)
         .then((res) => {
   ...
   const deleteData = (id) => {
      axios.delete(`/estateprops/${props.match.params.id}`).then((res) => {
